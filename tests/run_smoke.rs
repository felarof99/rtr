...
fn profile_overview_filters_usage_by_day_and_preserves_history_without_config() {
    ...
    assert!(all.contains("work enabled isolated 50% 2 1"), "{all}");
    assert!(all.contains("idle enabled isolated 50% 0 0"), "{all}");
    assert!(all.contains("removed removed - - 1 1"), "{all}");
    let today = run(&["ls"]);
    assert!(today.starts_with("rtr profiles · today"), "{today}");
    assert!(today.contains("work enabled isolated 50% 1 0"), "{today}");
